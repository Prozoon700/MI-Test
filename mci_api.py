...
def create_app(mc, api_token: str, drive_path: str, lightnode_url: str = "", panel_url: str = "") -> FastAPI:
    ...
    def list_backups():
        ...
        items = []
        for d_item in sorted(bp.rglob("*"), key=lambda x:x.stat().st_mtime, reverse=True):
            if d_item.is_dir() and d_item.parent in (bp, bp/"world"):
                sz = sum(f.stat().st_size for f in d_item.rglob("*") if f.is_file())
                items.append({"name":d_item.name,"size":_fmt_size(sz),
                    "date":time.strftime("%d %b %Y %H:%M",time.localtime(d_item.stat().st_mtime))})
